...
describe('Department', () =>{
  ...
  describe('Reading data', () => {
    ...
    it('should return a proper by. "name" with "findOe" method', async () =>{
  ...
  describe('Updating data', () => {
    ...
    it('should properly update one document with "save" method', async () => {
      const department = await Department.findOne({ name: 'Department #1' });
      department.name = 'Department #1 Update';
      await department.save();
    
      const updatedDepartment = await Department.findOne({ name: 'Department #1 Update' });
      expect(updatedDepartment).to.not.be.null;
    });
    ...
    it('should properly update multiple documents with "updateMany" method', async () => {
      await Department.updateMany({}, { $set: { name: 'update' }});
      const departments = await Department.find();
      expect(departments[0].name).to.be.equal('update');
      expect(departments[1].name).to.be.equal('update');
    });
